...
from decorator import bd_connection, logger, handle_errors
...
def get_validated_user_data(data):
    if isinstance(data[0], int):
        name, surname = validate_full_name(data[1])
        name, surname = validate_full_name(name + ' ' + surname)
        return name, surname, data[2], data[3]
    else:
        name, surname = validate_full_name(data[0])
        name, surname = validate_full_name(name + ' ' + surname)
